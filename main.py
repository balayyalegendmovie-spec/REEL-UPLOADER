...
GEMINI:      Frame selection (1 call per episode) + caption generation.
ANTI-DETECT: Smart jitter, random delays, session-only login,
             slight zoom/brightness shift, varied captions.
PRIVACY:     All temp files in /tmp/, drive_id as keys (no filenames
             in public repo), secrets never logged.
============================================================
"""
...
# ── GIT (subprocess only, no os.system) ───────────────────────
def git_cmd(*args):
    try:
...
# ── SMART JITTER (analyzes last 5 days, never repeats pattern) ─
def smart_delay():
...
# ── CAPTIONS (Gemini first, fallback templates) ───────────────
def generate_caption(display_name, part, total):
    """
    Try Gemini for unique viral caption every time.
    If Gemini fails (quota/error), use fallback templates.
    Gemini generates in regional language with trending hashtags.
    """
    lang = C.LANGUAGE
    p = part
    t = total
    ...
    if GEMINI and C.GEMINI_KEY:
        try:
            ...
            prompt = (
                ...
                f"Style: {style}\n\n"
                f"STRICT RULES:\n"
                f"1. Write the main text in {lang} script (Telugu/Tamil/Hindi native script, NOT English transliteration)\n"
...
def ig_login():
    ...
    try:
        ...
        # Random delay before verification (anti-detection)
...
def ig_upload(cl, clip_path, thumb_path, caption):
    ...
    for attempt in range(1, 4):
        try:
            # Random delay before upload (human-like behavior)
...
# ── V1 MIGRATION (runs once if old format detected) ──────────
def migrate_from_v1(drive_files):
    old_log = load_json(C.LOG, {})
    old_progress = load_json(C.PROGRESS, {})

    if "movies" not in old_log:
        return False

    if "order" in old_log and "videos" in old_log:
        sample_keys = list(old_log.get("videos", {}).keys())[:1]
        if sample_keys and not any(sample_keys[0].endswith(ext) for ext in C.VIDEO_EXTS):
            return False

    log("🔄 MIGRATING V1 → V2...")
    name_to_id = {f["name"]: f["id"] for f in drive_files}

    new_log = {"videos": {}, "order": [f["id"] for f in drive_files], "completed": 0, "uploaded": 0}

    for filename, info in old_log.get("movies", {}).items():
        drive_id = info.get("drive_id") or name_to_id.get(filename)
        if not drive_id:
            continue
        new_log["videos"][drive_id] = {
            "status": info.get("status", "pending"),
            "total_parts": info.get("total_parts", 0),
            "parts_done": info.get("ig_uploaded_parts", 0),
            "errors": info.get("errors", 0),
            "started": info.get("started_at", ""),
            "completed_at": info.get("completed_at", ""),
        }
        log(f"  Migrated: {filename[:50]} → {info.get('status')}")

    save_json(C.LOG, new_log)

    old_name = old_progress.get("movie_name", "")
    if old_name:
        drive_id = None
        for fn, info in old_log.get("movies", {}).items():
            if fn == old_name:
                drive_id = info.get("drive_id") or name_to_id.get(fn)
                break
        if drive_id:
            save_json(C.PROGRESS, {
                "drive_id": drive_id,
                "part": old_progress.get("last_uploaded", 0),
                "total": old_progress.get("total_parts", 0),
                "thumb_time": -1, "cooldown_until": ""
            })
            log(f"  Progress: Part {old_progress.get('last_uploaded', 0)}")

    for f in ["detailed_log.txt", "current_movie.mp4", "session.json", "yt_token.json"]:
        if os.path.exists(f):
            try: os.remove(f)
            except: pass
    for d in ["reels", "thumbnails", "tmp_frames", "thumb_cache"]:
        if os.path.exists(d):
            try: shutil.rmtree(d)
            except: pass
...
def main():
    ...
    migrated = migrate_from_v1(drive_files)
    if migrated:
        progress = load_progress()
